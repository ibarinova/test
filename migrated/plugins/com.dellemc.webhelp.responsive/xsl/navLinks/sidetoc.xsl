					...
					<xsl:variable name="publicationToc">
						<div data-type="temporary">
							<span class="expand-button-action-labels">
								<span id="{$expandActionID}" role="button" tabindex="0" aria-label="Expand" class="hidden">
								...
								<span id="{$collapseActionID}" role="button" tabindex="0" aria-label="Collapse" class="hidden">
								<!--	IB: Add support for TOC 'Expand all' / 'Collapse all' buttons. Add tooltips with localized value-->
									<span class="wh-tooltip">
										<xsl:call-template name="getVariable">
...
<!--		</xsl:if>-->
	...
	<xsl:template match="xhtml:li" mode="toc-accessibility">
		<xsl:copy>
			<xsl:variable name="state" select="xhtml:span[@data-state][1]/@data-state"/>
