...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*[contains(@class, ' map/topicref ')][not(ancestor::*[contains(@class, ' map/reltable ')])][@href][not(ends-with(@href, '.png'))]" mode="conrefs">
    ...
    <xsl:template match="*[contains(@class, ' map/topicref ') and @href][not(ends-with(@href, '.png'))]">
        <xsl:variable name="href" select="@href"/>
        <xsl:variable name="href-value">
            <xsl:choose>
                <xsl:when test="contains($href, '#')">
