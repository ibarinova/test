...
<!--					<xsl:apply-templates select="." mode="gen-user-head-child-elements"/>-->
			...
			<script type="text/javascript" src="./oxygen-webhelp/app/dell-topic-title-meta.js"><!----></script>
			<!-- IDPL-15171 Client DOC XSS & Open Redirect issue fix added by 986204 by Dell -->
			<script type="text/javascript" src="./oxygen-webhelp/app/dell-custom.js"><!----></script>
		</xsl:copy>
	</xsl:template>
...
<!--
	<xsl:template match="whc:webhelp_logo" mode="copy_template">
		<xsl:if test="string-length($WEBHELP_LOGO_IMAGE) > 0">
			...
			&lt;!&ndash; If the target of the logo is not specified, use the relative path to the index.html page. &ndash;&gt;
