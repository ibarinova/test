...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:import href="plugin:org.dita.post:xsl/get-meta.xsl"/>
  <xsl:import href="plugin:org.dita.post:xsl/rel-links.xsl"/>
  <xsl:import href="plugin:org.dita.post:xsl/glossdisplay.xsl"/>
<xsl:import href="plugin:org.dita.post:xsl/simpletable.xsl"/>
<xsl:import href="plugin:org.dita.post:xsl/css-class.xsl"/>
  <xsl:import href="plugin:org.dita.post:xsl/functions.xsl"/>
...
<!--       <xsl:call-template name="commonattributes">
  ...
  <xsl:template match="*[contains(@class, ' topic/topic ')]/*[contains(@class, ' topic/title ')]">
    ...
    <xsl:element name="h{$headinglevel+1}">
...
<!--<xsl:call-template name="commonattributes"/>-->
  ...
  <xsl:template match="*[contains(@class, ' topic/p ')][contains(@outputclass, 'pdfonly')]" priority="1"/>  
  <!-- Added for DITA 1.1 "Shortdesc proposal" -->
...
<!--      <xsl:call-template name="commonattributes"/>
  ...
  <!-- RFE 2703335 reduces duplicated code by adding common processing rules.
	   ...
	   <!--Removed processing of admonition other than note,caution and warning -->
...
<!--      <xsl:call-template name="commonattributes">
  ...
  <xsl:template match="*[contains(@class, ' topic/object ')]" name="topic.object">
   <object>
    <xsl:copy-of select="@height | @classid | @data | @width | @name"/>
...
<!--       <xsl:call-template name="commonattributes"/>
  ...
  <xsl:template name="commonattributes">
    ...
    <xsl:if test="exists($passthrough-attrs)">
      <xsl:for-each select="@*">
        ...
        <xsl:attribute name="data-{name()}" select="encode-for-uri(.)"/>
  ...
  <xsl:template match="*" mode="dita2html:section-heading">
    ...
    <xsl:variable name="headCount" select="count(ancestor::*[contains(@class, ' topic/topic ')]) + 2"/>
...
<!-- Removed <div> and language as per edell request -->
<!--<div>
...
<!--    <xsl:call-template name="setid"/>-->
  ...
  <!-- Process <body> content that is appropriate for post header section. -->
  ...
  <xsl:template name="generateBreadcrumbs">
    <!-- Insert previous/next/ancestor breadcrumbs links at the top of the post. -->
...
<!--      <xsl:call-template name="setid"/>-->
  ...
  <xsl:include href="tables_custom.xsl"/>
  ...
  <xsl:template match="*" mode="process.note.common-processing">
    ...
    <xsl:param name="title">
      <xsl:call-template name="getVariable">
        <xsl:with-param name="id" select="concat('dellps_',$type)"/>
      </xsl:call-template>
    </xsl:param>
    <div>
     <xsl:call-template name="commonattributes">
...
<!--       <xsl:call-template name="setidaname"/>-->
      ...
      <span class="note__title">
        <xsl:copy-of select="upper-case($title)"/>
        <!-- Removed  colon in stylesheet-->
      </span>
      <xsl:text> </xsl:text>
