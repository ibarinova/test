...
<xsl:stylesheet version="2.0"
  ...
  <xsl:key name="linkdup"
	...
	<xsl:param name="NOPARENTLINK" select="'yes'" as="xs:string"/><!-- "no" and "yes" are valid values; non-'no' is ignored -->
  ...
  <xsl:template match="*[contains(@class, ' topic/xref ')]" name="topic.xref">
    <xsl:choose>
      <xsl:when test="@href and normalize-space(@href) or starts-with(@href, '#')">
...
Each child is indented, the linktext is bold, and the shortdesc appears in normal text directly below the link, to create a summary-like appearance.-->
    ...
    <xsl:if test="exists($links)">
      <linklist class="- topic/linklist " outputclass="relinfo">
        <title class="- topic/title ">
          <xsl:call-template name="getVariable">
            <xsl:with-param name="id" select="'dellps_Related information'"/>
          </xsl:call-template>
        </title>
  ...
  <xsl:template match="*" mode="determine-final-href">
    <xsl:choose>
      ...
      <xsl:when test="starts-with(@href, '#')">
	   <xsl:value-of select="dita-ot:get-topic-id(@href)"/>
		<xsl:text>#</xsl:text> 
    ...
    and process the rest of the href -->
      <xsl:when test="(empty(@scope) or @scope = ('local', 'peer')) and (empty(@format) or @format = 'dita')">
        <xsl:call-template name="replace-extension">
		  ...
		  <!--removed extention, since edell do not required .html extension in href-->
        ...
        <xsl:if test="contains(@href, '#')">
         ...
         <!-- <xsl:value-of select="dita-ot:generate-id(dita-ot:get-topic-id(@href), dita-ot:get-element-id(''))"/>-->
        </xsl:if>
  ...
  <xsl:variable name="pubname"><xsl:value-of select="document(concat('file:///',$inputdir,'/',$pubguid,'.met'))//ishfield[@name='FTITLE']"/></xsl:variable>
<xsl:variable name="topiclang"><xsl:value-of select="document(concat('file:///',$job))//parameter[@name='language']"/></xsl:variable>  
  <xsl:if test="@href and normalize-space(@href)">
      <xsl:attribute name="href">
	  <xsl:if test="not(@scope = 'external')">
		<xsl:value-of select="concat($pubname,'/',$topiclang,'/')"/>
        <xsl:apply-templates select="." mode="determine-final-href"/>
      </xsl:if>
