...
<!--      <xsl:call-template name="commonattributes"/>-->
      <xsl:apply-templates select="@compact"/>
		<xsl:choose>
			<!-- <xsl:when test="$olcount mod 3 = 1"/> --> <!-- previous code, commented since 4th level should not be numeric -->
			<!-- <xsl:when test="$olcount mod 3 = 2"> -->  <!-- previous code, commented since 5th level should not be numeric -->
			<xsl:when test="$olcount = 1"/> <!-- use numeric for the first level -->
			<xsl:when test="$olcount = 2">  <!-- use lower-case letters for the second level -->
			    <xsl:attribute name="type">a</xsl:attribute>
			</xsl:when>
			<xsl:otherwise> <!-- use roman numerals for the third level and up -->
...
<!--    <xsl:call-template name="setid"/>-->
  ...
  <!-- Process <body> content that is appropriate for plainhtml header section. -->
  ...
  <xsl:template name="generateBreadcrumbs">
    <!-- Insert previous/next/ancestor breadcrumbs links at the top of the plainhtml. -->
