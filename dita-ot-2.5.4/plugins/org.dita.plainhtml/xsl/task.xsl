...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!--get task label value from bookmap-->
  <xsl:param name="ditamap"/>
  <xsl:param name="GENERATE-TASK-LABELS">
  <xsl:choose><xsl:when test="document(concat('file:///',$ditamap))//othermeta[@name = 'task-labels']">YES</xsl:when><xsl:otherwise>NO</xsl:otherwise>
  </xsl:choose>
  </xsl:param>
...
<!--    <xsl:call-template name="commonattributes"/>
  ...
  <!--<xsl:template match="*[contains(@class,' task/prereq ')]" mode="get-output-class">prereq</xsl:template>
  <xsl:template match="*[contains(@class,' task/context ')]" mode="get-output-class">context</xsl:template>
    <xsl:template match="*[contains(@class,' task/postreq ')]" mode="get-output-class">postreq</xsl:template>
	<xsl:template match="*[contains(@class,' task/steps ')]" mode="get-output-class">steps</xsl:template>-->
      ...
      <xsl:with-param name="use-label">
        <xsl:call-template name="getVariable">
          <xsl:with-param name="id">
            <xsl:choose>
              <xsl:when test="contains(@class,' task/steps ')">dellps_task_procedure</xsl:when>
   ...
   <!-- If there's a block element somewhere in the step list, expand the whole list -->
    ...
    <ol>
      <xsl:if test="parent::*/parent::*[contains(@class,' task/steps ')]"> <!-- Is the grandparent an ordered step? -->
		  <!--<xsl:attribute name="class">steps</xsl:attribute>-->
		  <!-- commented since steps class is empty and all other plugins use directly type=a -->
...
<!--      <xsl:call-template name="commonattributes"/>
  ...
  <xsl:template match="*[contains(@class,' task/prereq ')]" mode="dita2html:section-heading">
    <xsl:apply-templates select="." mode="generate-task-label">
      <xsl:with-param name="use-label">
        <xsl:call-template name="getVariable">
          <xsl:with-param name="id" select="'dellps_task_prereq'"/>
        </xsl:call-template>
      </xsl:with-param>
    </xsl:apply-templates>
  </xsl:template>
  ...
  <xsl:template match="*[contains(@class,' task/context ')]" mode="dita2html:section-heading">
    <xsl:apply-templates select="." mode="generate-task-label">
      <xsl:with-param name="use-label">
        <xsl:call-template name="getVariable">
          <xsl:with-param name="id" select="'dellps_task_context'"/>
        </xsl:call-template>
      </xsl:with-param>
    </xsl:apply-templates>
  </xsl:template>
  ...
  <xsl:template match="*[contains(@class,' task/result ')]" mode="dita2html:section-heading">
    <xsl:apply-templates select="." mode="generate-task-label">
      <xsl:with-param name="use-label">
        <xsl:call-template name="getVariable">
          <xsl:with-param name="id" select="'dellps_task_results'"/>
        </xsl:call-template>
      </xsl:with-param>
    </xsl:apply-templates>
  </xsl:template>
  ...
  <xsl:template match="*[contains(@class,' task/postreq ')]" mode="dita2html:section-heading">
    <xsl:apply-templates select="." mode="generate-task-label">
      <xsl:with-param name="use-label">
        <xsl:call-template name="getVariable">
          <xsl:with-param name="id" select="'dellps_task_postreq'"/>
        </xsl:call-template>
      </xsl:with-param>
    </xsl:apply-templates>
  </xsl:template>
  ...
  <xsl:template match="*[contains(@class,' task/taskbody ')]/*[contains(@class,' topic/example ')][not(*[contains(@class,' topic/title ')])]" mode="dita2html:section-heading">
    <xsl:apply-templates select="." mode="generate-task-label">
      <xsl:with-param name="use-label">
        <xsl:call-template name="getVariable">
          <xsl:with-param name="id" select="'dellps_task_example'"/>
        </xsl:call-template>
      </xsl:with-param>
    </xsl:apply-templates>
  </xsl:template>
  ...
  <xsl:template match="*" mode="generate-task-label">
    ...
    <xsl:if test="$GENERATE-TASK-LABELS='YES'">
      <xsl:variable name="headLevel">
        <xsl:variable name="headCount" select="count(ancestor::*[contains(@class,' topic/topic ')]) + 3"
          as="xs:integer"/>
        <xsl:choose>
    ...
    <xsl:if test="normalize-space(string-join($links, ''))">
      <linklist class="- topic/linklist " outputclass="relinfo reltasks">
        <title class="- topic/title ">
          <xsl:call-template name="getVariable">
            <xsl:with-param name="id" select="'dellps_Related tasks'"/>
          </xsl:call-template>
        </title>
