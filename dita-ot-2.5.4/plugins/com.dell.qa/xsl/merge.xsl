...
<xsl:param name="src-file"></xsl:param>
  ...
  <xsl:template match="@id" mode="copy-element">
    <xsl:attribute name="id"><xsl:choose><xsl:when test="parent::resourceid"><xsl:value-of select="."/></xsl:when><xsl:otherwise><xsl:value-of select="generate-id(.)"/></xsl:otherwise></xsl:choose></xsl:attribute>
  </xsl:template>
