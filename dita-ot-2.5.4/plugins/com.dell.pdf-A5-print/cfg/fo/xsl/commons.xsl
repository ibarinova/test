...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*" mode="processUnknowTopic"
        ...
        <xsl:choose>
            <xsl:when test="empty(ancestor::*[contains(@class,' topic/topic ')]) and empty(ancestor::ot-placeholder:glossarylist)">
                <xsl:if test="$multilingual and $isManual">
                    <xsl:call-template name="createFrontMatterMultilingualInside-manual"/>
                </xsl:if>
            ...
            <xsl:otherwise>
                <xsl:if test="$multilingual and ($topicType = 'topicChapter')">
                    <xsl:if test="not($isManual) and ancestor-or-self::*[contains(@class, ' topic/topic ')][last()][preceding-sibling::*[contains(@class, ' topic/topic ')]]">
                        <fo:block xsl:use-attribute-sets="break.page"/>
                    </xsl:if>
                    <xsl:variable name="current-topicref" select="key('map-id', @id)"/>
                    <xsl:if test="$isManual and $current-topicref[preceding-sibling::*[contains(@class, ' bookmap/chapter ')]]">
                        <fo:block xsl:use-attribute-sets="break.page.odd"/>
                    </xsl:if>
                    <xsl:if test="not(preceding-sibling::*[contains(@class,' topic/topic ')])">
    ...
    <xsl:template name="insert_admonition_image">
        ...
        <fo:block xsl:use-attribute-sets="note__image">
            <fo:external-graphic src="url({$image-path})" content-height="18px"/>
        </fo:block>
    </xsl:template>
