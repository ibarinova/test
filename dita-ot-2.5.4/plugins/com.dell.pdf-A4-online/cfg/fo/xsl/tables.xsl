...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="*[contains(@class, ' topic/table ')]/*[contains(@class,' topic/desc ')]" mode="from-tgroup">
        <xsl:apply-templates/>
    </xsl:template>
    ...
    <xsl:template name="generateTableEntryBorder">
		...
		<!--Added border bottom for table break IDPL13028-->
        <xsl:if test="$needTopBorderOnBreak">
            <xsl:call-template name="processAttrSetReflection">
                <xsl:with-param name="attrSet" select="'__tableframe__top','__tableframe__bottom_Pagebreak'"/>
                <xsl:with-param name="path" select="$tableAttrs"/>
            </xsl:call-template>
        </xsl:if>
                ...
                <fo:table table-omit-footer-at-break="true">
                    ...
                    <xsl:if test="preceding-sibling::*[contains(@class,' topic/title ')]">
                        <fo:table-header>
                            ...
                            <xsl:if test="preceding-sibling::*[contains(@class, ' topic/desc ')]">
                                <xsl:variable name="desc">
                                    <xsl:apply-templates select="preceding-sibling::*[contains(@class, ' topic/desc ')]" mode="from-tgroup"/>
                                </xsl:variable>
                                <fo:table-row height="1px" padding="0px" line-height="1px">
                                    <fo:marker marker-class-name="continued"/>
                                    <fo:table-cell>
                                        <fo:block>
                                            <fo:external-graphic src="url({concat($artwork-dir, 'pixel.png')})">
    ...
    <!--Styling column header IDPL13028-->
    ...
    <xsl:template match="*[contains(@class, ' topic/simpletable ')]">
        ...
        <fo:table xsl:use-attribute-sets="simpletable">
            ...
            <xsl:if test="ancestor::*[contains(@class, ' topic/li ')]">
