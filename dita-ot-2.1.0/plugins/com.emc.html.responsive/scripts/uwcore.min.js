...
s.migrateWarnings=[];
if(document.location.port!=""||document.location.href.indexOf("jQueryMigrateMute=false")>0){s.migrateMute=false
}else{s.migrateMute=true
}if(!s.migrateMute&&p.console&&p.console.log){p.console.log("JQMIGRATE: Logging is active on environments using port numbers");
p.console.log("           Mimic other environemnts, by setting parameter jQueryMigrateMute to false")
}if(s.migrateTrace===i){s.migrateTrace=true
}s.migrateReset=function(){D={};
s.migrateWarnings.length=0
};
function h(H){var G=p.console;
if(!D[H]){D[H]=true;
s.migrateWarnings.push(H);
if(G&&G.warn&&!s.migrateMute){G.warn("JQMIGRATE: "+H);
if(s.migrateTrace&&G.trace){G.trace()
}}}}function a(I,K,H,J){if(Object.defineProperty){try{Object.defineProperty(I,K,{configurable:true,enumerable:true,get:function(){h(J);
return H
},set:function(L){h(J);
H=L
}});
return
}catch(G){}}s._definePropertyBroken=true;
I[K]=H
}if(document.compatMode==="BackCompat"){h("jQuery is not compatible with Quirks Mode")
}var f=s("<input/>",{size:1}).attr("size")&&s.attrFn,x=s.attr,w=s.attrHooks.value&&s.attrHooks.value.get||function(){return null
},j=s.attrHooks.value&&s.attrHooks.value.set||function(){return i
...
s.attr=function(K,I,L,J){var H=I.toLowerCase(),G=K&&K.nodeType;
if(J){if(x.length<4){h("jQuery.fn.attr( props, pass ) is deprecated")
}if(K&&!y.test(G)&&(f?I in f:s.isFunction(s.fn[I]))){return s(K)[I](L)
}}if(I==="type"&&L!==i&&t.test(K.nodeName)&&K.parentNode){h("Can't change the 'type' of an input or button in IE 6/7/8")
}if(!s.attrHooks[H]&&B.test(H)){s.attrHooks[H]={get:function(N,M){var P,O=s.prop(N,M);
return O===true||typeof O!=="boolean"&&(P=N.getAttributeNode(M))&&P.nodeValue!==false?M.toLowerCase():i
},set:function(N,P,M){var O;
if(P===false){s.removeAttr(N,M)
}else{O=s.propFix[M]||M;
if(O in N){N[O]=true
}N.setAttribute(M,M.toLowerCase())
}return M
}};
if(k.test(H)){h("jQuery.fn.attr('"+H+"') may use property instead of attribute")
}}return x.call(s,K,I,L)
};
s.attrHooks.value={get:function(H,G){var I=(H.nodeName||"").toLowerCase();
if(I==="button"){return w.apply(this,arguments)
}if(I!=="input"&&I!=="option"){h("jQuery.fn.attr('value') no longer gets properties")
}return G in H?H.value:null
},set:function(G,H){var I=(G.nodeName||"").toLowerCase();
if(I==="button"){return j.apply(this,arguments)
}if(I!=="input"&&I!=="option"){h("jQuery.fn.attr('value', val) no longer sets properties")
}G.value=H
}};
var q,E,z=s.fn.init,A=s.parseJSON,v=/^([^<]*)(<[\w\W]+>)([^>]*)$/;
s.fn.init=function(G,J,I){var H;
if(G&&typeof G==="string"&&!s.isPlainObject(J)&&(H=v.exec(s.trim(G)))&&H[0]){if(G.charAt(0)!=="<"){h("$(html) HTML strings must start with '<' character")
}if(H[3]){h("$(html) HTML text after last tag is ignored")
}if(H[0].charAt(0)==="#"){h("HTML string cannot start with a '#' character");
s.error("JQMIGRATE: Invalid selector string (XSS)")
}if(J&&J.context){J=J.context
}if(s.parseHTML){return z.call(this,s.parseHTML(H[2],J,true),J,I)
}}return z.apply(this,arguments)
};
s.fn.init.prototype=s.fn;
s.parseJSON=function(G){if(!G&&G!==null){h("jQuery.parseJSON requires a valid JSON string");
return null
}return A.apply(this,arguments)
};
s.uaMatch=function(H){H=H.toLowerCase();
...
if(!s.browser){q=s.uaMatch(navigator.userAgent);
E={};
if(q.browser){E[q.browser]=true;
E.version=q.version
}if(E.chrome){E.webkit=true
}else{if(E.webkit){E.safari=true
}}s.browser=E
}a(s,"browser",s.browser,"jQuery.browser is deprecated");
s.sub=function(){function G(J,K){return new G.fn.init(J,K)
}s.extend(true,G,this);
G.superclass=this;
G.fn=G.prototype=this();
G.fn.constructor=G;
G.sub=this.sub;
G.fn.init=function I(J,K){if(K&&K instanceof s&&!(K instanceof G)){K=G(K)
...
var n=s.fn.data;
s.fn.data=function(I){var H,G,J=this[0];
if(J&&I==="events"&&arguments.length===1){H=s.data(J,I);
G=s._data(J,I);
if((H===i||H===G)&&G!==i){h("Use of jQuery.fn.data('events') is deprecated");
return G
}}return n.apply(this,arguments)
};
var o=/\/(java|ecma)script/i,u=s.fn.andSelf||s.fn.addBack;
s.fn.andSelf=function(){h("jQuery.fn.andSelf() replaced by jQuery.fn.addBack()");
return u.apply(this,arguments)
};
if(!s.clean){s.clean=function(G,H,N,J){H=H||document;
H=!H.nodeType&&H[0]||H;
H=H.ownerDocument||H;
h("jQuery.clean() is deprecated");
var K,I,L,O,M=[];
s.merge(M,s.buildFragment(G,H).childNodes);
if(N){L=function(P){if(!P.type||o.test(P.type)){return J?J.push(P.parentNode?P.parentNode.removeChild(P):P):N.appendChild(P)
}};
for(K=0;
(I=M[K])!=null;
K++){if(!(s.nodeName(I,"script")&&L(I))){N.appendChild(I);
if(typeof I.getElementsByTagName!=="undefined"){O=s.grep(s.merge([],I.getElementsByTagName("script")),L);
M.splice.apply(M,[K+1,0].concat(O));
K+=O.length
}}}}return M
}
}var c=s.event.add,b=s.event.remove,g=s.event.trigger,r=s.fn.toggle,d=s.fn.live,m=s.fn.die,C="ajaxStart|ajaxStop|ajaxSend|ajaxComplete|ajaxError|ajaxSuccess",e=new RegExp("\\b(?:"+C+")\\b"),F=/(?:^|\s)hover(\.\S+|)\b/,l=function(G){if(typeof(G)!=="string"||s.event.special.hover){return G
}if(F.test(G)){h("'hover' pseudo-event is deprecated, use 'mouseenter mouseleave'")
}return G&&G.replace(F,"mouseenter$1 mouseleave$1")
};
if(s.event.props&&s.event.props[0]!=="attrChange"){s.event.props.unshift("attrChange","attrName","relatedNode","srcElement")
}if(s.event.dispatch){a(s.event,"handle",s.event.dispatch,"jQuery.event.handle is undocumented and deprecated")
}s.event.add=function(J,H,I,K,G){if(J!==document&&e.test(H)){h("AJAX events should be attached to document: "+H)
}c.call(this,J,l(H||""),I,K,G)
};
s.event.remove=function(K,I,J,G,H){b.call(this,K,l(I)||"",J,G,H)
};
s.fn.error=function(){var G=Array.prototype.slice.call(arguments,0);
h("jQuery.fn.error() is deprecated");
G.splice(0,0,"error");
if(arguments.length){return this.bind.apply(this,G)
}this.triggerHandler.apply(this,G);
return this
};
s.fn.toggle=function(K,I){if(!s.isFunction(K)||!s.isFunction(I)){return r.apply(this,arguments)
}h("jQuery.fn.toggle(handler, handler...) is deprecated");
var H=arguments,G=K.guid||s.guid++,J=0,L=function(M){var N=(s._data(this,"lastToggle"+K.guid)||0)%J;
s._data(this,"lastToggle"+K.guid,N+1);
M.preventDefault();
return H[N].apply(this,arguments)||false
};
L.guid=G;
while(J<H.length){H[J++].guid=G
}return this.click(L)
};
s.fn.live=function(G,I,H){h("jQuery.fn.live() is deprecated");
if(d){return d.apply(this,arguments)
}s(this.context).on(G,this.selector,I,H);
return this
};
s.fn.die=function(G,H){h("jQuery.fn.die() is deprecated");
if(m){return m.apply(this,arguments)
}s(this.context).off(G,this.selector||"**",H);
return this
};
s.event.trigger=function(I,J,H,G){if(!H&&!e.test(I)){h("Global events are undocumented and deprecated")
}return g.call(this,I,J,H||document,G)
};
s.each(C.split("|"),function(H,G){s.event.special[G]={setup:function(){var I=this;
if(I!==document){s.event.add(document,G+"."+s.guid,function(){s.event.trigger(G,null,I,true)
});
s._data(this,G,s.guid++)
}return false
},teardown:function(){if(this!==document){s.event.remove(document,G+"."+s._data(this,G))
}return false
}}
})
})(jQuery,window);
...
Modernizr.addTest("cookies",function(){if(navigator.cookieEnabled){return true
...
(function(){var b=document.createElement("a"),a=b.style,c="right 10px bottom 10px";
Modernizr.addTest("bgpositionshorthand",function(){a.cssText="background-position: "+c+";";
return(a.backgroundPosition===c)
})
}());
...
Modernizr.addTest("cssfilters",function(){var a=document.createElement("div");
a.style.cssText=Modernizr._prefixes.join("filter:blur(2px); ");
return !!a.style.length&&((document.documentMode===undefined||document.documentMode>9))
});
Modernizr.addTest("bgpositionxy",function(){return Modernizr.testStyles("#modernizr {background-position: 3px 5px;}",function(c){var d=window.getComputedStyle?getComputedStyle(c,null):c.currentStyle;
var b=(d.backgroundPositionX=="3px")||(d["background-position-x"]=="3px");
var a=(d.backgroundPositionY=="5px")||(d["background-position-y"]=="5px");
return b&&a
})
});
Modernizr.testStyles("#modernizr{background-size:cover}",function(b){var a=window.getComputedStyle?window.getComputedStyle(b,null):b.currentStyle;
Modernizr.addTest("bgsizecover",a.backgroundSize=="cover")
});
(function(){function a(b){return(window.getComputedStyle?getComputedStyle(b,null).getPropertyValue("background"):b.currentStyle.background)
}Modernizr.testStyles(" #modernizr { background-repeat: round; } ",function(b,c){Modernizr.addTest("bgrepeatround",a(b)=="round")
});
Modernizr.testStyles(" #modernizr { background-repeat: space; } ",function(b,c){Modernizr.addTest("bgrepeatspace",a(b)=="space")
})
})();
...
Modernizr.addTest("forceios",function(){return window.location.href.indexOf("forceios")>=0
});
Modernizr.addTest("appleios",function(){return(Modernizr.ipad||Modernizr.ipod||Modernizr.iphone||Modernizr.forceios)
});
Modernizr.addTest("android",function(){return !!navigator.userAgent.match(/android/i)
});
Modernizr.addTest("mobile",function(){return(Modernizr.touch||Modernizr.android||Modernizr.forceios)
});
Modernizr.addTest("ie",function(){if(navigator.userAgent.indexOf("MSIE")!==-1||navigator.appVersion.indexOf("Trident/")>0){var docRoot=document.documentElement;
if(docRoot.className.indexOf(" ie")!=-1){return docRoot.id.replace("ie","")*1
}else{var addedClasses=" gt-ie9 gt-ie8 gt-ie7 gt-ie6 ";
...
Modernizr.addTest("csscalc",function(){var c="width:";
var b="calc(10px);";
var a=document.createElement("div");
a.style.cssText=c+Modernizr._prefixes.join(b+c);
...
(function(){window.UW=window.UW||{};
UW.util={cookies:{set:function(b,e,a,f,d){var c=b+"="+escape(e)+";";
if(a){if(a instanceof Date){if(isNaN(a.getTime())){a=new Date()
}}else{a=new Date(new Date().getTime()+parseInt(a)*1000*60*60*24)
}c+="expires="+a.toGMTString()+";"
}if(f){c+="path="+f+";"
}if(d){c+="domain="+d+";"
}document.cookie=c
},get:function(c){var a=new RegExp("[; ]"+c+"=([^\\s;]*)");
var b=(" "+document.cookie).match(a);
if(c&&b){return unescape(b[1])
}return""
},remove:function(a,c,b){if(this.get(a)){createCookie(a,"",-1,c,b)
}}},docWrite:function(b,c){var c=(c!="async")?c:"";
for(var a=0;
a<b.length;
a++){document.write("<script data-sync "+c+' src="'+b[a]+'" ><\/script>')
}return
},extend:function(c,b){var a={};
var d;
for(d in c){if(Object.prototype.hasOwnProperty.call(c,d)){a[d]=c[d]
}}for(d in b){if(Object.prototype.hasOwnProperty.call(b,d)){a[d]=b[d]
}}return a
},hash:(function(){var a=function(b){var d="";
for(var c in b){if(b.hasOwnProperty(c)){if(b[c]!==null&&c){if(d!==""){d+="&"
}d+=c+"="+b[c]
}}}return d
};
return{getAll:function(g){g=(g||window.location.hash);
if(!g){return{}
}var b=g.split("&");
b[0]=b[0].replace("#","");
var f={};
...
d<b.length;
d++){var c=b[d].split("=")[0];
var e=b[d].indexOf("=")>=0?b[d].split("=")[1]:"";
f[c]=e
}return f
},get:function(c){var b=this.getAll();
return b[c]?b[c]:false
},set:function(c,d){var b=this.getAll();
b[c]=d;
window.location.hash=a(b)
},remove:function(c){var b=this.getAll();
if(b[c]){b[c]=null
}window.location.hash=a(b)
},removeAll:function(){window.location.hash=""
}}
})(),deepLink:(function(c){var b={};
var e=0;
var d=window.location.hash;
var a=function(f){if(f&&typeof f.callBack=="function"){if(f.self){f.callBack.call(f.self)
}else{f.callBack()
}}};
c(window).load(function(){var f=5;
if(Modernizr.ie){f=5000
}setTimeout(function(){var i=[];
var g=UW.util.hash.getAll();
for(var h in g){if(b[h]&&c.inArray(b[h].id,i)==-1){a(b[h]);
i.push(b[h].id)
}}},f)
});
c(window).on("hashchange",function(){var g=window.location.hash;
var h=d;
if(g){g=UW.util.hash.getAll(g)
}else{g={}
}if(h){h=UW.util.hash.getAll(h)
}else{h={}
}for(var f in g){if(g[f]!=h[f]){a(b[f])
}}for(f in h){if(typeof g[f]=="undefined"){a(b[f])
}}d=window.location.hash
});
return function(g,h,j){if(typeof g=="object"){if(g.length){for(var f=0;
f<g.length;
f++){b[g[f]]={};
b[g[f]].callBack=h;
b[g[f]].id=e;
...
})(jQuery),namespace:function(d){var c=window,b=d.split("."),a;
while(b.length>0){a=b.shift();
if(typeof c[a]==="undefined"){c[a]={}
}c=c[a]
}return c
}}
})();
(function(){UW.util.namespace("UW.url");
UW=UW.util.extend(UW,{url:(function(){var a=typeof(a)=="undefined"?window.location:UW.url;
return{href:(function(){return a.href
})(),host:(function(){return a.host
})(),hostname:(function(){return a.hostname
})(),hostscript:(function(){var b=document.getElementsByTagName("script");
return b[b.length-1].src.split("/")[2]
})(),hash:(function(){return a.hash
})(),origin:(function(){return a.origin
})(),pathname:(function(){return a.pathname
})(),port:(function(){return a.port
})(),protocol:(function(){return a.protocol
})(),search:(function(){return a.search
})()}
})()})
})();
(function(a){window.UW=window.UW||{};
UW.homepage={displayNextElement:function(c,d){c=a(c);
if(Modernizr.mobile||!c.size()){return false
}var b={displayHeight:100,elementMinHeight:542,textElement:".leadspace-header",breakpoint:UW.breakpoint.breakpoints.large||960};
b=a.extend(b,d);
var e=function(){if(a(window).height()>b.elementMinHeight&&a(window).height()<b.elementMinHeight+c.offset().top+b.displayHeight){var f=c.find(b.textElement);
if(f){if(f.height()>a(window).height()-c.offset().top-b.displayHeight){c.css("height","");
return false
}}c.height(a(window).height()-c.offset().top-b.displayHeight)
}else{c.css("height","")
}};
UW.breakpoint.onEnter(function(){window.setTimeout(e,100);
a(window).resize(e)
},b.breakpoint);
UW.breakpoint.onExit(function(){a(window).off("resize",e);
window.setTimeout(function(){c.css("height","")
},100)
},b.breakpoint)
},cookieRedirection:function(){window.ip=window.ip||"";
if(UW.url.host.indexOf("emc.com")>0&&ip.indexOf("128.221.24")<0&&ip.indexOf("10.24.3")<0&&!UW.url.search){var b=UW.util.cookies.get("EMCSiteUrl");
if(b.indexOf("http")==0){b=b.replace("http://","").replace("https://","");
UW.util.cookies.set("EMCSiteUrl",unescape(b),"Fri, 17 Dec 2020 10:00:00 GMT","/","emc.com")
}if(UW.url.search.indexOf("?fromGlobalSiteSelect")==-1&&!b){window.location="/utilities/globalsiteselect.jhtml?checked=true"
}else{if(UW.url.href.indexOf(b)==-1){window.location=UW.url.protocol+"//"+b
}}}}}
})(jQuery);
(function(){UW.util.namespace("UW.breakpoint");
UW=UW.util.extend(UW,{breakpoint:(function(){var fakeEndRange=Number.MAX_VALUE;
var listenerSet=false;
var viewportChangeListenerAdded=false;
var enterRanges=[];
var exitRanges=[];
var breakpoints={small:0,medium:640,large:960,xlarge:1280};
var previousBreakpoint="";
var viewportWidth=window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;
var addResizeListener=function(){if(!listenerSet){jQuery(window).resize(function(event){viewportWidth=window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;
for(var i=0;
i<enterRanges.length;
i++){processEnterRange(enterRanges[i])
}for(var i=0;
i<exitRanges.length;
i++){processExitRange(exitRanges[i])
}});
listenerSet=true
}};
var processEnterRange=function(range){if(!range.executing&&(viewportWidth>=range.startRange&&viewportWidth<range.endRange)){eval(range.callback());
range.executing=true
}else{if(range.executing&&(viewportWidth<range.startRange||viewportWidth>=range.endRange)){range.executing=false
}}};
var processExitRange=function(range){if(!range.executing&&(viewportWidth<range.startRange||viewportWidth>=range.endRange)){eval(range.callback());
range.executing=true
}else{if(range.executing&&(viewportWidth>=range.startRange&&viewportWidth<range.endRange)){range.executing=false
}}};
var addEnterCallback=function(callback,startRange,endRange){var range={};
...
var addExitCallback=function(callback,startRange,endRange){var range={};
range.startRange=startRange;
range.endRange=endRange;
range.executing=false;
range.callback=callback;
...
var getScreenSize=function(){var lastSize=undefined;
for(var screenSize in breakpoints){var viewport=window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth;
if(viewport<breakpoints[screenSize]){screenSize=lastSize;
break
}lastSize=screenSize
}return screenSize
};
var triggerBreakpointChange=function(){var currentBreakpoint=getScreenSize(),oldBreakpoint=previousBreakpoint;
if(currentBreakpoint!==previousBreakpoint){previousBreakpoint=currentBreakpoint;
jQuery(window).triggerHandler("breakpoint.changed",{exit:oldBreakpoint,enter:currentBreakpoint})
}};
var addBreakpointChangeListener=function(){var deferred=new jQuery.Deferred();
if(!viewportChangeListenerAdded){jQuery(window).resize(function(event){triggerBreakpointChange()
});
setTimeout(function(){triggerBreakpointChange()
},50);
viewportChangeListenerAdded=true
}deferred.resolve();
return deferred.promise()
};
return{breakpoints:breakpoints,onEnter:function(callback,startRange,endRange){addResizeListener();
addEnterCallback(callback,startRange,arguments.length===2?fakeEndRange:endRange)
},onExit:function(callback,startRange,endRange){addResizeListener();
addExitCallback(callback,startRange,arguments.length===2?fakeEndRange:endRange)
},getScreenSize:getScreenSize,addBreakpointChangeListener:addBreakpointChangeListener}
})()})
})();
UW.util.docWrite(["//assets.adobedtm.com/81d0c75cc2c56ee7cbbb3de103fe1f7b442bb9a7/satelliteLib-c3c3c7ad9967d3bb40540600d09c43ef34628abc.js"]);