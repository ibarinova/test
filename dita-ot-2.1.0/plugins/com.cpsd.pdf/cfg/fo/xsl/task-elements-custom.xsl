...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*[contains(@class, ' task/substeps ')]/*[contains(@class, ' task/substep ')]">
        ...
        <fo:list-item xsl:use-attribute-sets="substeps.substep">
            ...
            <fo:list-item-label xsl:use-attribute-sets="substeps.substep__label">
				<!-- Dimitri: Render numbers in Dell blue. -->
	...
	<!-- Dimitri: Added stepsection rendering. -->
    ...
    <!-- Dimitri: Stepsection elements should not have a number and should restart numbering. -->
    <xsl:template match="*[contains(@class, ' task/steps ')]/*[contains(@class, ' task/step ')]" priority="100">
        ...
        <fo:list-item xsl:use-attribute-sets="steps.step">
                                ...
                                <number>
                                    <xsl:choose>
                                        <xsl:when test="preceding-sibling::*[contains(@class, ' task/stepsection ')]">
                                            <xsl:number level="any" from="*[contains(@class, 'task/stepsection')]"/>
                                        </xsl:when>
                                        <xsl:otherwise>
            ...
            <fo:list-item-body xsl:use-attribute-sets="steps.step__body">
                <xsl:variable name="countSubsteps" select="count(*[contains(@class, ' task/substeps ')])"/>
                <xsl:choose>
                    <xsl:when test="$countSubsteps = 0 or $countSubsteps &gt; 1">
                    ...
                    <xsl:otherwise>
                        <fo:block xsl:use-attribute-sets="steps.step__content">
                            <xsl:apply-templates select="*[contains(@class, ' task/substeps ')]/preceding-sibling::*"/>
                        </fo:block>
                        <fo:block>
                            <xsl:apply-templates select="*[contains(@class, ' task/substeps ')]"/>
                        </fo:block>
                        <xsl:if test="*[contains(@class, ' task/substeps ')]/following-sibling::*">
                            <fo:block xsl:use-attribute-sets="steps.step__content">
                                <xsl:apply-templates select="*[contains(@class, ' task/substeps ')]/following-sibling::*"/>
                            </fo:block>
                        </xsl:if>
                    </xsl:otherwise>
                </xsl:choose>
